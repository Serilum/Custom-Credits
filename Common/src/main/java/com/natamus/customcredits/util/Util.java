	...
	public static void createDefaultFiles() throws IOException {
		if (!dir.isDirectory() || !creditsFile.isFile() || !poemFile.isFile() || !postCreditsFile.isFile()) {
			...
			if (!creditsFile.isFile()) {
				Reader creditsReader = mc.getResourceManager().openAsReader(Identifier.fromNamespaceAndPath("minecraft", "texts/credits.json"));
			...
			if (!poemFile.isFile()) {
				Reader poemReader = mc.getResourceManager().openAsReader(Identifier.fromNamespaceAndPath("minecraft", "texts/end.txt"));
			...
			if (!postCreditsFile.isFile()) {
				Reader postCreditsReader = mc.getResourceManager().openAsReader(Identifier.fromNamespaceAndPath("minecraft", "texts/postcredits.txt"));
